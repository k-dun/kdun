...
const ProjectCard = ({ project }) => {
  return (
    <div className="w-[90svw] md:w-[70vw] flex flex-col rounded pt-6">
      ...
      <div className="w-full flex flex-row py-8 pr-8 pl-3 md:pl-0 justify-around md:justify-start">
        <Link href={project.codeUrl} target="_blank">
          <button className="w-[38svw] md:w-[26vw] font-semibold text-[#202020] bg-[#FCFAFF] text-base md:text-lg px-5 py-3 rounded-md border border-[#202020] justify-center mr-2 md:mr-4">Code</button>
        </Link>
        <Link href={project.liveUrl} target="_blank">
          <button className="w-[38svw] md:w-[26vw] font-semibold text-[#202020] bg-[#FCFAFF] text-base md:text-lg px-5 py-3 rounded-md border border-[#FF8080] flex flex-row justify-center">Link</button>
        </Link>
      </div>
    </div>
  );
